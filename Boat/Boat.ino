 ...
 * the motor drive is with a rellay board
 * the stearing drive is with a rellay board
 * communication is via the RC 433MHZ
 */

 /*
 * Phase 3:
 * Setup main boat arduino board 
 * the motor drive is with a PWM board and a relay
 * the stearing drive is with a servo motor and requires a PWM
 * communication is via the NRF24
 */
...
#define STEAR_MOTOR_POWER PWM
  ...
  #define STEAR_MOTOR_PIN 5 // this pin is with PWM
  ...
  #define MAIN_MOTOR_PIN 3 // this pin is with PWM
  #define MAIN_DIRECTION_PIN 2
#endif
...
void update_motor_values(byte value_motor, byte value_stear)
{
  motor[MAIN_MOTOR]= value_motor;
  motor[STEAR_MOTOR]= value_stear;
...
int set_drive_state(){
    ...
    byte servo_angle = 90;
    if (motor[STEAR_MOTOR] <= 100 ) 
      servo_angle=map(motor[STEAR_MOTOR],0,100,SERVO_STRAIGHT_ANGLE,60);
    else
      servo_angle=map(motor[STEAR_MOTOR],101,200,SERVO_STRAIGHT_ANGLE,120);
...
void setup() {
  ...
  #endif
  #if STEAR_MOTOR_POWER == PWM
    myservo.attach(STEAR_MOTOR_PIN);  // attaches the servo on pin 5 to the servo object
    myservo.write(90);  // move the servo to the middle
  #endif
  #if MAIN_MOTOR_POWER == RELAY
...
void loop() {
    ...
    if (nrf24.available())
    {
      ...
      if (receive_buf[0] == MY_ID) {
        update_motor_values(receive_buf[1], receive_buf[2]);
      }
      digitalWrite(RECEIVE_LED_PIN,LOW);
    }
  #endif
